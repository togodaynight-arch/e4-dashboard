...
function proxyRequest(req, res) {
    ...
    req.on('end', () => {
        ...
        proxyReq.on('error', (err) => {
            ...
            res.end(JSON.stringify({ error: 'Erro na conexao com a API' }));
        });
...
function portalLogin() {
    return new Promise(function(resolve, reject) {
        ...
        https.get(API_BASE + '/central/index/login', function(res1) {
            ...
            if (!phpsess) { reject('Nao obteve cookie de sessao'); return; }
                ...
                res2.on('end', function() {
                    try {
                        ...
                        if (j.sMensagem === true) {
                            ...
                            console.log('Login E4 OK, sessao valida ate ' + new Date(sessExpires).toLocaleTimeString());
                            resolve(phpsess);
                        } else {
                            reject('Login falhou: ' + (j.sMensagem || 'desconhecido'));
                        }
                    } catch(e) {
                        reject('Erro ao parsear resposta login');
                    }
                });
            });
            req2.on('error', function(e) { reject('Erro login: ' + e.message); });
            req2.write(postData);
            req2.end();
        }).on('error', function(e) { reject('Erro ao obter sessao: ' + e.message); });
    });
}
...
function fetchOcorrencias() {
    return new Promise(async function(resolve, reject) {
        ...
        [1,2,3].forEach(function(id) {
            postData += '&filtro-filtro-id_lojas%5B%5D=' + id;
        });
        OCCURRENCE_TYPES.forEach(function(t) {
            postData += '&filtro-filtro-ws_ocorrencias_frente_caixa%5B%5D=' + t;
        });
        ...
        var req = https.request(opts, function(res) {
            ...
            res.on('end', function() {
                try {
                    ...
                    reject('Erro ao parsear ocorrencias');
                }
            });
        });
        req.on('error', function(e) { reject('Erro rede: ' + e.message); });
        req.write(postData);
        req.end();
    });
}
        ...
        .then(function(dados) {
            var simplified = dados.map(function(o) {
                return {
                    ...
                    pdv: o.pdv,
                    cupom: o.cupom,
                    valor: o.cupons_vl_liquido,
                    transacao: o.transacao,
                    cancelado: o.cupons_cancelado,
        ...
        var req = https.request(opts, function(res) {
            ...
            res.on('end', function() {
                try {
                    ...
                    reject('Erro ao parsear entradas');
                }
            });
        });
        req.on('error', function(e) { reject('Erro rede: ' + e.message); });
        ...
        .then(function(dados) {
            var simplified = dados.map(function(o) {
                return {
                    ...
                    obs: o.obs,
                    trava: o.app_porta_cad_tmp_loja,
...
const server = http.createServer((req, res) => {
    ...
    if (req.url.startsWith('/ocorrencias')) { handleOcorrencias(req, res); }
    else if (req.url.startsWith('/entradas-porta')) { handleEntradasPorta(req, res); }
    else if (req.url.startsWith('/api/')) { proxyRequest(req, res); }
    else { serveStatic(req, res); }
});

server.listen(PORT, () => {
    console.log('');
    console.log('  E4 Conciliacao');
    console.log('  ====================');
    console.log('');
